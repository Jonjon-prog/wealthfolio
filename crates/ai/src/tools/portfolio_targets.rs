...
pub struct GetPortfolioTargetsArgs {
    /// Account ID, or "PORTFOLIO" for the entire portfolio.
...
pub struct DeviationDto {
    ...
    /// Positive = need to buy, negative = overweight.
    pub value_delta: f64,
    pub is_locked: bool,
}
...
impl<E: AiEnvironment + 'static> Tool for GetPortfolioTargetsTool<E> {
    ...
    async fn definition(&self, _prompt: String) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Get portfolio allocation targets and how the current portfolio deviates from them. Returns each target with per-category target%, current%, deviation%, and value delta (positive = need to buy more, negative = overweight). Use this to answer questions about rebalancing needs or how aligned the portfolio is with its targets.".to_string(),
